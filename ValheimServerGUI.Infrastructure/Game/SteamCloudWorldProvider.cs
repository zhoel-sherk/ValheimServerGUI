...
namespace ValheimServerGUI.Game
{
    ...
    {
        /// <summary>
        /// Marks a world that lives only in Steam Cloud. The UI appends this suffix to cloud worlds in
        /// the world dropdown and strips it before import, so it never reaches validation or saved prefs.
        /// </summary>
        ...
        private IEnumerable<DirectoryInfo> GetRemoteFolders()
        {
            ...
            {
                var remote = Path.Join(account, ValheimConstants.SteamAppId, "remote");
        ...
        private string GetSteamPath()
        {
            ...
            // The Steam install path lives in the Windows registry; degrade to local-only elsewhere.
            if (!OperatingSystem.IsWindows()) return null;

#pragma warning disable CA1416 // Registry is Windows-only; guarded by the check above
            try
            {
